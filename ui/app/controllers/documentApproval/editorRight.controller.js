sap.ui.controller('app.controllers.documentApproval.editorRight', {
    onInit: function() {
        var _self = this;
    },
            ...
            buttons: [{
                ...
                click: function() {
                    ...
                    if(_self.coreServices.getComponents("validFrom")){
                        ...
                        item.validFrom = from.getDate ? parseDate(from.getDate(), 'initDate')  : null;
                    }
                    if(_self.coreServices.getComponents("validTo")){
                        var to = _self.coreServices.getComponents("validTo");
                        item.validTo = to.getDate ? parseDate(to.getDate(), 'initDate')  : null;
                    }
                    if(_self.coreServices.getComponents("approvalDate")){
                        var approval = _self.coreServices.getComponents("approvalDate");
                        item.approvalDate = approval.getDate ? parseDate(approval.getDate(), 'initDate')  : null;
                    }
                    ...
                    if (_self.coreServices.action === 'copy') {
                        delete item.id;
                    }
                    // console.log(item)
                    Data.endpoints.documentApproval[endpoint].post(item).success(function(_data) {
                        $.baseToast({
                        ...
                        if ((_self.coreServices.action === 'new' || _self.coreServices.action === 'copy') && !saveAndNew ) {
                            _self.getLock({
    ...
    renderTabs: function() {
        ...
        this.view.tabs.ctrl = this.view.tabs.bindBaseTabs({
            tab: [{
                viewName: "app.views.documentApproval.configuration"
            }],
    ...
    validate: function(item) {
        ...
        var config = item.docTypeId !== null && item.fileStoredId !== "" && item.comments !== "" && item.status !== null;
        ...
        var approvalDate = _self.coreServices.getComponents("approvalDate");

        if (!_self.validateDate(_self.coreServices.item.validFrom, _self.coreServices.item.validTo) && (!validTo.isValid() || !validFrom.isValid() || !approvalDate.isValid())) {
            isValid = false;
            $.baseToast({
